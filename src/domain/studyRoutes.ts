import type { ContentInputType, SessionGenerationContext, StudyRouteKind } from "../api/contracts";
...
export type StudyFocusMode = "reading" | "writing" | "expression";

export interface DynamicRouteCopy {
  introGoals: string[];
  readingTitle: string;
...
export function inferStudyRouteKind(params: {
  contentType: ContentInputType;
  focusLabel?: string;
  fileName?: string;
  text?: string;
}): StudyRouteKind {
  const { contentType, focusLabel = "", fileName = "", text = "" } = params;
...
export function inferStudyRouteKindFromGenerationContext(
  context?: Partial<SessionGenerationContext>,
): StudyRouteKind {
...
export function inferStudyFocusMode(recognizedFocus = ""): StudyFocusMode {
...
export function getStudyRouteMeta(routeKind: StudyRouteKind) {
  switch (routeKind) {
    case "reading_quiz":
      return {
        routeLabel: "阅读题突破路线",
...
export function buildStudyRouteSummary(routeKind: StudyRouteKind, lessonTitle?: string, skillLabel?: string) {
...
export function getStudyContentTypeLabel(contentType: ContentInputType) {
  switch (contentType) {
    case "textbook":
      return "教材";
    case "worksheet":
...
export function shouldIncludeRecitationCard(params: {
  contentType: ContentInputType;
  focusMode: StudyFocusMode;
  routeKind: StudyRouteKind;
  snippet: string;
}) {
  const { contentType, focusMode, routeKind, snippet } = params;
...
export function buildDynamicRouteCopy(params: {
  routeKind: StudyRouteKind;
  contentType: ContentInputType;
  focusMode: StudyFocusMode;
  sourceTitle: string;
  ...
  const { routeKind, contentType, focusMode, sourceTitle, sourceSummary, canRecite } = params;
  const routeMeta = getStudyRouteMeta(routeKind);
...
export function estimateStudyRouteTaskCount(params: {
  contentType: ContentInputType;
  recognizedFocus: string;
  routeKind: StudyRouteKind;
  recognizedTextSnippet?: string;
}) {
  const { contentType, recognizedFocus, routeKind, recognizedTextSnippet = "" } = params;
